(function (root, factory) {
  ...
  Module((Tone) => {
	    ...
	    Tone.Timeline = function () {
	        const options = Tone.defaults(arguments, ['memory'], Tone.Timeline);
	        Tone.call(this);
	        /**
	    ...
	    Tone.Timeline.prototype.get = function (time, comparator) {
	        ...
	        const index = this._search(time, comparator);
	        if (index !== -1) {
	    ...
	    Tone.Timeline.prototype._search = function (time, comparator) {
	        ...
	        while (beginning < end) {
	            ...
	            if (event[comparator] === time) {
	                ...
	                return midPoint;
	            } if (event[comparator] < time && nextEvent[comparator] > time) {
	                return midPoint;
	            } if (event[comparator] > time) {
	                // search lower
	                end = midPoint;
	    ...
	    Tone.Timeline.prototype._iterate = function (callback, lowerBound, upperBound) {
	        ...
	        this._iterating = false;
	        this._toRemove.forEach((event) => {
	            this.remove(event);
	        });
	        this._toRemove = [];
	        this._toAdd.forEach((event) => {
	            this.add(event);
	        });
	        this._toAdd = [];
	    };
  ...
  Module((Tone) => {
	    ...
	    Tone.Context = function () {
	        ...
	        const options = Tone.defaults(arguments, ['context'], Tone.Context);
	        if (!options.context) {
	    ...
	    Tone.Context.prototype.dispose = function () {
	        ...
	        this._timeouts = null;
	        for (const con in this._constants) {
	    ...
	    Ticker.prototype._createWorker = function () {
	        ...
	        const blob = new Blob([// the initial timeout time
  ...
  Module((Tone) => {
	    ...
	    Tone.AudioNode = function () {
	        ...
	        const options = Tone.defaults(arguments, ['context'], { context: Tone.context });
	        /**
	    ...
	    Tone.TimeBase = function (val, units) {
	        ...
	        if (this instanceof Tone.TimeBase) {
	            ...
	            if (val instanceof Tone.TimeBase) {
	                ...
	                const { method } = this._primaryExpressions[units];
  ...
  Module((Tone) => {
	    ...
	    Tone.prototype.toSeconds = function (time) {
	        if (Tone.isNumber(time)) {
	            ...
	            return this.now();
	        } if (Tone.isString(time)) {
	            return new Tone.Time(time).toSeconds();
	        } if (time instanceof Tone.TimeBase) {
	    ...
	    Tone.prototype.toFrequency = function (freq) {
	        if (Tone.isNumber(freq)) {
	            return freq;
	        } if (Tone.isString(freq) || Tone.isUndef(freq)) {
	            return new Tone.Frequency(freq).valueOf();
	        } if (freq instanceof Tone.TimeBase) {
  ...
  Module((Tone) => {
		 ...
		 */
	    Tone.Param = function () {
	        const options = Tone.defaults(arguments, [
	            'param',
	            'units',
	            'convert',
	        ], Tone.Param);
  ...
  Module((Tone) => {
	    ...
	    Object.defineProperty(Tone.Envelope.prototype, 'attackCurve', {
	        get() {
	            ...
	            } if (Tone.isArray(this._attackCurve)) {
	                // look up the name in the curves array
	                for (const type in Tone.Envelope.Type) {
	        ...
	        set(curve) {
	            ...
	            if (Tone.Envelope.Type.hasOwnProperty(curve)) {
	                const curveDef = Tone.Envelope.Type[curve];
	    ...
	    Object.defineProperty(Tone.Envelope.prototype, 'releaseCurve', {
	        get() {
	            ...
	            } if (Tone.isArray(this._releaseCurve)) {
	                // look up the name in the curves array
	                for (const type in Tone.Envelope.Type) {
	    ...
	    Tone.Envelope.prototype.triggerAttack = function (time, velocity) {
	        ...
	        if (this._attackCurve === 'linear') {
	            ...
	            if (attack < originalAttack) {
	                const percentComplete = 1 - attack / originalAttack;
	                const sliceIndex = Math.floor(percentComplete * this._attackCurve.length);
  ...
  Module((Tone) => {
	    ...
	    Tone.AudioToGain = function () {
			 ...
			 *  @private
			 */
	        this._norm = this.input = this.output = new Tone.WaveShaper(((x) => (x + 1) / 2));
	    };
  ...
  Module(function (Tone) {
	    ...
	    Tone.Expr.prototype._parseTree = function (expr) {
	        ...
	        const isUndef = Tone.isUndef.bind(this);
  ...
  Module((Tone) => {
		 ...
		 */
	    Tone.FFT = function () {
	        const options = Tone.defaults(arguments, ['size'], Tone.FFT);
  ...
  Module((Tone) => {
	    ...
	    Tone.Follower.prototype._setAttackRelease = function (attack, release) {
	        const minTime = this.blockTime;
	        attack = Tone.Time(attack).toFrequency();
	        release = Tone.Time(release).toFrequency();
	        attack = Math.max(attack, minTime);
	        release = Math.max(release, minTime);
	        this._frequencyValues.setMap((val) => {
	            if (val <= 0) {
	                return attack;
	            }
	                return release;
  ...
  Module((Tone) => {
	    ...
	    Tone.ScaledEnvelope = function () {
	        // get all of the defaults
	        let options = Tone.defaults(arguments, [
	            'attack',
	            'decay',
	            'sustain',
	            'release',
  ...
  Module((Tone) => {
	    ...
	    function _wrapScheduleMethods(method) {
	        return function (value, time) {
	            ...
	            const ticksUntilTime = this._getTickUntilEvent(previousEvent, time - this.sampleTime);
	    ...
	    Tone.TickSignal.prototype.setTargetAtTime = function (value, time, constant) {
	        ...
	        for (let i = 0; i <= segments; i++) {
	            ...
	            const rampVal = this._exponentialApproach(prevEvent.time, prevEvent.value, value, constant, segTime);
	    ...
	    Tone.TickSignal.prototype.exponentialRampToValueAtTime = function (value, time) {
	        ...
	        value = this._fromUnits(value);
	        // start from previously scheduled value
	        let prevEvent = this._events.get(time);
	        if (prevEvent === null) {
	            prevEvent = {
	                value: this._initial,
	                time: 0,
	        ...
	        for (let i = 0; i <= segments; i++) {
	            ...
	            const rampVal = this._exponentialInterpolate(prevEvent.time, prevEvent.value, time, value, segTime);
	            this.linearRampToValueAtTime(this._toUnits(rampVal), segTime);
	        }
	        return this;
	    };
	    ...
	    Tone.TickSignal.prototype._getTickUntilEvent = function (event, time) {
	        ...
	        const val1 = this.getValueAtTime(time);
	    ...
	    Tone.TickSignal.prototype.getDurationOfTicks = function (ticks, time) {
	        time = this.toSeconds(time);
	        const currentTick = this.getTickAtTime(time);
	    ...
	    Tone.TickSignal.prototype.getTimeOfTick = function (tick) {
	        ...
	        if (before && before.ticks === tick) {
	            return before.time;
	        } if (before && after && after.type === Tone.TimelineSignal.Type.Linear && before.value !== after.value) {
  ...
  Module((Tone) => {
	    ...
	    Tone.Clock.prototype._loop = function () {
	        ...
	        const endTime = this.now() + this.context.updateInterval;
	        // the current event at the time of the loop
	        const event = this._state.get(endTime);
	        if (event) {
	            // state change events
  ...
  Module((Tone) => {
	    ...
	    Tone.Transport.prototype.nextSubdivision = function (subdivision) {
	        ...
	        const transportPos = Tone.Time(this.ticks, 'i');
  ...
  Module((Tone) => {
	    ...
	    Tone.Volume = function () {
	        const options = Tone.defaults(arguments, ['volume'], Tone.Volume);
	        Tone.AudioNode.call(this);
	        /**
			 * the output node
			 * @type {GainNode}
  ...
  Module((Tone) => {
		 ...
		 */
	    const MasterConstructor = Tone.Master;
  ...
  Module((Tone) => {
	    ...
	    Tone.Source.prototype.start = function (time, offset, duration) {
	        ...
	        if (this._synced) {
	            ...
	            const sched = Tone.Transport.schedule((t) => {
	    ...
	    Tone.Source.prototype.stop = function (time) {
	        ...
	        if (!this._synced) {
	            ...
	            const sched = Tone.Transport.schedule(this._stop.bind(this), time);
	            this._scheduled.push(sched);
	        }
	        return this;
	    };
  ...
  Module((Tone) => {
	    ...
	    Object.defineProperty(Tone.Oscillator.prototype, 'type', {
	        ...
	        set(type) {
	            const coefs = this._getRealImaginary(type, this._phase);
	            const periodicWave = this.context.createPeriodicWave(coefs[0], coefs[1]);
	    ...
	    Tone.Oscillator.prototype._getInitialValue = function () {
	        const coefs = this._getRealImaginary(this._type, 0);
	        const real = coefs[0];
	        const imag = coefs[1];
	        let maxValue = 0;
  ...
  Module((Tone) => {
	    ...
	    Tone.Limiter = function () {
	        const options = Tone.defaults(arguments, ['threshold'], Tone.Limiter);
	        Tone.AudioNode.call(this);
	        /**
			 *  the compressor
			 *  @private
  ...
  Module((Tone) => {
		 ...
		 */
	    Tone.Meter = function () {
	        const options = Tone.defaults(arguments, ['smoothing'], Tone.Meter);
	        Tone.AudioNode.call(this);
	        /**
  ...
  Module((Tone) => {
		 ...
		 */
	    Tone.Solo = function () {
	        const options = Tone.defaults(arguments, ['solo'], Tone.Solo);
	        Tone.AudioNode.call(this);
	        /**
	    ...
	    Tone.Solo.prototype._removeSolo = function () {
	        if (this._isSoloed()) {
	            const index = this.context._currentSolo.indexOf(this);
  ...
  Module((Tone) => {
	    ...
	    Tone.Waveform = function () {
	        const options = Tone.defaults(arguments, ['size'], Tone.Waveform);
  ...
  Module((Tone) => {
	    ...
	    Object.defineProperty(Tone.CtrlInterpolate.prototype, 'value', {
	        get() {
	            ...
	            const lowerPosition = Math.floor(index);
	            const lower = this.values[lowerPosition];
	            const upper = this.values[Math.ceil(index)];
	    ...
	    Tone.CtrlInterpolate.prototype._interpolate = function (index, lower, upper) {
	        if (Tone.isArray(lower)) {
	            ...
	            return retArray;
	        } if (Tone.isObject(lower)) {
	            const retObj = {};
	            for (const attr in lower) {
  ...
  Module((Tone) => {
	    ...
	    Tone.CtrlMarkov.prototype.next = function () {
	        if (this.values.hasOwnProperty(this.value)) {
	            const next = this.values[this.value];
	            if (Tone.isArray(next)) {
	                const distribution = this._getProbDistribution(next);
  ...
  Module((Tone) => {
	    ...
	    Object.defineProperty(Tone.CtrlPattern.prototype, 'value', {
	        get() {
	            ...
	            let val = this.values[this.index];
  ...
  Module((Tone) => {
	    ...
	    Object.defineProperty(Tone.CtrlRandom.prototype, 'value', {
	        get() {
	            const min = this.toSeconds(this.min);
	            const max = this.toSeconds(this.max);
	            const rand = Math.random();
	            let val = rand * min + (1 - rand) * max;
	            if (this.integer) {
	                val = Math.floor(val);
	            }
	            return val;
  ...
  Module((Tone) => {
	    ...
	    if (window.AudioBuffer && !AudioBuffer.prototype.copyToChannel) {
	        AudioBuffer.prototype.copyToChannel = function (src, chanNum, start) {
	            const channel = this.getChannelData(chanNum);
	            start = start || 0;
	    ...
	    Tone.Buffer.prototype.load = function (url, onload, onerror) {
	        const promise = new Promise(((load, error) => {
	    ...
	    Tone.Buffer.prototype.fromArray = function (array) {
	        const isMultidimensional = array[0].length > 0;
	        const channels = isMultidimensional ? array.length : 1;
	        const len = isMultidimensional ? array[0].length : array.length;
	        const buffer = this.context.createBuffer(channels, len, this.context.sampleRate);
	    ...
	    Tone.Buffer.prototype.slice = function (start, end) {
	        ...
	        const startSamples = Math.floor(this.context.sampleRate * this.toSeconds(start));
	        const endSamples = Math.floor(this.context.sampleRate * this.toSeconds(end));
	        const replacement = [];
	        for (let i = 0; i < this.numberOfChannels; i++) {
	            replacement[i] = this.toArray(i).slice(startSamples, endSamples);
	        }
	        const retBuffer = new Tone.Buffer().fromArray(replacement);
	        return retBuffer;
	    };
	    /**
		 *  Reverse the buffer.
	    ...
	    Tone.Buffer._removeFromDownloadQueue = function (request) {
	        const index = Tone.Buffer._downloadQueue.indexOf(request);
	        if (index !== -1) {
	    ...
	    Tone.Buffer.load = function (url, onload, onerror) {
	        ...
	        function onProgress() {
	            ...
	            let totalProgress = 0;
	            for (let i = 0; i < Tone.Buffer._downloadQueue.length; i++) {
	        ...
	        request.addEventListener('load', () => {
	            if (request.status === 200) {
	                Tone.context.decodeAudioData(request.response, (buff) => {
	                    request.progress = 1;
	                    onProgress();
	                    onload(buff);
	                    Tone.Buffer._removeFromDownloadQueue(request);
	    ...
	    Tone.Buffer.cancelDownloads = function () {
	        Tone.Buffer._downloadQueue.slice().forEach((request) => {
	            Tone.Buffer._removeFromDownloadQueue(request);
	            request.abort();
	        });
	        return Tone.Buffer;
  ...
  Module((Tone) => {
	    ...
	    Tone.Buffers = function (urls) {
	        ...
	        args.shift();
	        const options = Tone.defaults(args, [
	            'onload',
	            'baseUrl',
	        ], Tone.Buffers);
	        Tone.call(this);
	    ...
	    Object.defineProperty(Tone.Buffers.prototype, 'loaded', {
	        get() {
	            let isLoaded = true;
	            for (const buffName in this._buffers) {
	                const buff = this.get(buffName);
  ...
  Module((Tone) => {
	    ...
	    Tone.prototype.send = function (channelName, amount) {
	        ...
	        const sendKnob = new Tone.Gain(amount, Tone.Type.Decibels);
  ...
  Module((Tone) => {
	    ...
	    Tone.Offline = function (callback, duration) {
	        ...
	        const context = new Tone.OfflineContext(2, duration, sampleRate);
  ...
  Module((Tone) => {
	    ...
	    Tone.Effect = function () {
	        const options = Tone.defaults(arguments, ['wet'], Tone.Effect);
	        Tone.AudioNode.call(this);
	        this.createInsOuts(1, 1);
	        /**
  ...
  Module((Tone) => {
	    ...
	    Tone.AutoPanner = function () {
	        const options = Tone.defaults(arguments, ['frequency'], Tone.AutoPanner);
	        Tone.Effect.call(this, options);
	        /**
  ...
  Module((Tone) => {
	    ...
	    Tone.BitCrusher = function () {
	        const options = Tone.defaults(arguments, ['bits'], Tone.BitCrusher);
	        Tone.Effect.call(this, options);
	        const invStepSize = 1 / Math.pow(2, options.bits - 1);
	        /**
	    ...
	    Object.defineProperty(Tone.BitCrusher.prototype, 'bits', {
	        ...
	        set(bits) {
	            this._bits = bits;
	            const invStepSize = 1 / Math.pow(2, bits - 1);
  ...
  Module((Tone) => {
	    ...
	    Tone.Chebyshev = function () {
	        const options = Tone.defaults(arguments, ['order'], Tone.Chebyshev);
	        Tone.Effect.call(this, options);
	        /**
			 *  @type {WaveShaperNode}
			 *  @private
  ...
  Module((Tone) => {
	    ...
	    Tone.FeedbackEffect = function () {
	        const options = Tone.defaults(arguments, ['feedback'], Tone.FeedbackEffect);
	        Tone.Effect.call(this, options);
	        /**
  ...
  Module((Tone) => {
	    ...
	    Tone.Convolver = function () {
	        ...
	        if (Tone.isString(options.url)) {
	            this._buffer.load(options.url, (buffer) => {
	                this.buffer = buffer;
	                options.onload();
	            });
	        } else if (options.url) {
	    ...
	    Tone.Convolver.prototype.load = function (url, callback) {
	        return this._buffer.load(url, (buff) => {
	            this.buffer = buff;
	            if (callback) {
	                callback();
	            }
  ...
  Module((Tone) => {
	    ...
	    Tone.Distortion = function () {
	        const options = Tone.defaults(arguments, ['distortion'], Tone.Distortion);
	        Tone.Effect.call(this, options);
	        /**
			 *  @type {Tone.WaveShaper}
			 *  @private
  ...
  Module((Tone) => {
	    ...
	    Tone.Freeverb = function () {
	        ...
	        for (let c = 0; c < combFilterTunings.length; c++) {
	            const lfpf = new Tone.LowpassCombFilter(combFilterTunings[c]);
  ...
  Module((Tone) => {
	    ...
	    Tone.JCReverb = function () {
	        const options = Tone.defaults(arguments, ['roomSize'], Tone.JCReverb);
	        Tone.StereoEffect.call(this, options);
	        /**
	        ...
	        for (let af = 0; af < allpassFilterFreqs.length; af++) {
	        ...
	        for (let cf = 0; cf < combFilterDelayTimes.length; cf++) {
	            const fbcf = new Tone.FeedbackCombFilter(combFilterDelayTimes[cf], 0.1);
	    ...
	    Tone.JCReverb.prototype.dispose = function () {
	        Tone.StereoEffect.prototype.dispose.call(this);
	        for (let apf = 0; apf < this._allpassFilters.length; apf++) {
	            this._allpassFilters[apf].disconnect();
	            this._allpassFilters[apf] = null;
	        }
	        this._allpassFilters = null;
	        for (let fbcf = 0; fbcf < this._feedbackCombFilters.length; fbcf++) {
  ...
  Module((Tone) => {
	    ...
	    Tone.Phaser.prototype.dispose = function () {
	        ...
	        this._lfoR.dispose();
	        this._lfoR = null;
	        for (let i = 0; i < this._filtersL.length; i++) {
	            this._filtersL[i].disconnect();
	            this._filtersL[i] = null;
	        }
	        this._filtersL = null;
	        for (let j = 0; j < this._filtersR.length; j++) {
  ...
  Module((Tone) => {
	    ...
	    Tone.PitchShift = function () {
	        const options = Tone.defaults(arguments, ['pitch'], Tone.PitchShift);
	        Tone.FeedbackEffect.call(this, options);
	        /**
			 *  The pitch signal
			 *  @type  {Tone.Signal}
  ...
  Module((Tone) => {
	    ...
	    Tone.StereoWidener = function () {
	        const options = Tone.defaults(arguments, ['width'], Tone.StereoWidener);
	        ...
	        this._state.forEachFrom(after, (event) => {
	            ...
	            if (event.state === Tone.State.Started) {
	                ...
	                if (this._loop) {
	                    ...
	                    const interval = Tone.Time(this._getLoopDuration(), 'i');
  ...
  Module((Tone) => {
	    ...
	    Tone.Part.prototype.at = function (time, value) {
	        ...
	        const tickTime = Tone.Time(1, 'i').toSeconds();
  ...
  Module((Tone) => {
	    ...
	    Tone.PulseOscillator = function () {
			 ...
			 *  @private
			 */
	        this._thresh = new Tone.WaveShaper(((val) => {
	            if (val < 0) {
	                return -1;
	            }
	                return 1;
  ...
  Module((Tone) => {
	    ...
	    Tone.OmniOscillator.prototype._createNewOscillator = function (oscType) {
	        if (oscType !== this._sourceType) {
	            ...
	            const OscillatorConstructor = Tone[oscType];
  ...
  Module((Tone) => {
	    ...
	    Tone.Monophonic.prototype.setNote = function (note, time) {
	        ...
	        if (this.portamento > 0) {
	            const currentNote = this.frequency.value;
  ...
  Module((Tone) => {
	    ...
	    Tone.MembraneSynth.prototype.triggerAttack = function (note, time, velocity) {
	        ...
	        const maxNote = note * this.octaves;
  ...
  Module((Tone) => {
	    ...
	    Tone.MetalSynth = function (options) {
	        ...
	        for (let i = 0; i < inharmRatios.length; i++) {
	            ...
	            this._oscillators[i] = osc;
	            const mult = new Tone.Multiply(inharmRatios[i]);
  ...
  Module((Tone) => {
	    ...
	    Tone.BufferSource.prototype.start = function (time, offset, duration, gain, fadeInTime) {
	        ...
	        if (this.buffer.loaded) {
	            ...
	            let computedDur = this.toSeconds(Tone.defaultArg(duration, this.buffer.duration - offset));
	            ...
	            if (this.loop) {
	                ...
	                const loopEnd = this.loopEnd || this.buffer.duration;
	                const { loopStart } = this;
	    ...
	    Tone.BufferSource.prototype.stop = function (time, fadeOutTime) {
	        if (this.buffer.loaded) {
	            ...
	            if (this._stopTime === -1 || this._stopTime > time) {
	                ...
	                // set a new one
	                const heldDuration = Math.min(time - this._startTime - this.fadeIn - this.sampleTime, this.buffer.duration);
	                fadeOutTime = Math.min(heldDuration, fadeOutTime);
	                const startFade = time - fadeOutTime;
  ...
  Module((Tone) => {
		 ...
		 */
	    Tone.Noise = function () {
	        const options = Tone.defaults(arguments, ['type'], Tone.Noise);
	        Tone.Source.call(this, options);
	        /**
			 *  @private
			 *  @type {AudioBufferSourceNode}
	    ...
	    const _noiseArrays = {
	        pink: (function () {
	            ...
	            for (let channelNum = 0; channelNum < channels; channelNum++) {
	                const channel = new Float32Array(bufferLength);
	                buffer[channelNum] = channel;
	                var b0; var b1; var b2; var b3; var b4; var b5; var
            b6;
	    ...
	    function createBuffers() {
	        for (const type in _noiseArrays) {
  ...
  Module((Tone) => {
	    ...
	    Tone.PolySynth = function () {
	        ...
	        for (let i = 0; i < options.polyphony; i++) {
	            const v = new options.voice(arguments[2], arguments[3]);
	            this.voices[i] = v;
	    ...
	    Tone.PolySynth.prototype.triggerAttack = function (notes, time, velocity) {
	        ...
	        for (let i = 0; i < notes.length; i++) {
	            ...
	            let oldest = this._triggers[0];
	            let oldestIndex = 0;
	            for (let j = 1; j < this._triggers.length; j++) {
	    ...
	    Tone.PolySynth.prototype.triggerRelease = function (notes, time) {
	        ...
	        for (let i = 0; i < notes.length; i++) {
	            // get the voice
	            const stringified = JSON.stringify(notes[i]);
	            for (let v = 0; v < this._triggers.length; v++) {
	                const desc = this._triggers[v];
  ...
  Module((Tone) => {
	    ...
	    Tone.Sampler = function (urls) {
	        ...
	        Tone.Instrument.call(this, options);
	        const urlMap = {};
	        for (const note in urls) {
	            if (Tone.isNote(note)) {
	                // convert the note name to MIDI
	                const mid = Tone.Frequency(note).toMidi();
	    ...
	    Tone.Sampler.prototype.triggerRelease = function (note, time) {
	        ...
	        if (this._activeSources[midi] && this._activeSources[midi].length) {
	            const { source } = this._activeSources[midi].shift();
	            time = this.toSeconds(time);
	    ...
	    Tone.Sampler.prototype.dispose = function () {
	        ...
	        this._buffers = null;
	        for (const midi in this._activeSources) {
	            this._activeSources[midi].forEach((event) => {
  ...
  Module((Tone) => {
	    ...
	    Tone.GainToAudio = function () {
			 ...
			 *  @private
			 */
	        this._norm = this.input = this.output = new Tone.WaveShaper(((x) => Math.abs(x) * 2 - 1));
	    };
  ...
  Module((Tone) => {
	    ...
	    Tone.TransportTimelineSignal.prototype._anchorValue = function (time) {
	        const val = this.getValueAtTime(Tone.Transport.ticks);
	        this._lastVal = val;
  ...
  Module((Tone) => {
	    ...
	    Tone.MultiPlayer.prototype._makeSource = function (bufferName) {
	        ...
	        const source = new Tone.BufferSource(buffer).connect(this.output);
	    ...
	    Tone.MultiPlayer.prototype.start = function (bufferName, time, offset, duration, pitch, gain) {
	        time = this.toSeconds(time);
	        const source = this._makeSource(bufferName);
	    ...
	    Tone.MultiPlayer.prototype.stopAll = function (time) {
	        time = this.toSeconds(time);
	        for (const bufferName in this._activeSources) {
	            const sources = this._activeSources[bufferName];
	    ...
	    Tone.MultiPlayer.prototype.dispose = function () {
	        Tone.Source.prototype.dispose.call(this);
	        for (const bufferName in this._activeSources) {
	            this._activeSources[bufferName].forEach((source) => {
	                source.dispose();
	            });
	        }
  ...
  Module((Tone) => {
	    ...
	    Tone.Players = function (urls) {
	        const args = Array.prototype.slice.call(arguments);
	        args.shift();
	        const options = Tone.defaults(args, ['onload'], Tone.Players);
	        Tone.call(this);
	        /**
			 *  The output volume node
			 *  @type  {Tone.Volume}
  ...
  Module((Tone) => {
	    ...
	    Tone.UserMedia = function () {
	        const options = Tone.defaults(arguments, ['volume'], Tone.UserMedia);
	        Tone.AudioNode.call(this);
	        /**
			 *  The MediaStreamNode
	    ...
	    Tone.UserMedia.prototype.open = function (labelOrId) {
	        ...
	        return Tone.UserMedia.enumerateDevices().then((devices) => {
	            ...
	            if (Tone.isNumber(labelOrId)) {
	                ...
	                device = devices.find((device) => device.label === labelOrId || device.deviceId === labelOrId);
